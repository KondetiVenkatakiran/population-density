...
public class InteractionPane extends JComponent {
    ...
    // Flag to prevent recursive events when setting text programmatically
    ...
    public InteractionPane(final JFrame appFrame){
        ...
        // add the program Versions sub-panel to the Interaction Pane
        add(programVersions);
        ...
        // Add an editor (label + text field) for the number of columns
        ...
        // add sub-panel to the Interaction Pane
    ...
    public void initMapGrid(int rows, int columns, PropertyChangeListener listener){
        mapGrid = new MapGrid();
        mapGrid.addPropertyChangeListener(listener);
        mapGrid.setRows(rows);
        mapGrid.setColumns(columns);
        setNumRowsAndColumnsFields(rows, columns);
        mapGrid.fireChange();
    }
    
    private void setNumRowsAndColumnsFields(int rows, int columns){
    ...
    public static class MapGrid {
        ...
        public void setRows(int rows){
            ...
            if(oldRows != rows){
                this.rows = rows;
                pcs.firePropertyChange("rows", oldRows, rows);
            }
        }
        ...
        public void setColumns(int columns){
            ...
            if(oldColumns != columns){
                ...
                pcs.firePropertyChange("columns", oldColumns, columns);
            }
        }
        ...
        public void fireChange(){
            pcs.firePropertyChange("rows", null, rows);
            pcs.firePropertyChange("columns", null, columns);
        }
    }
